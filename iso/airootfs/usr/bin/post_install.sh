...
_copy_files(){
    ...
    local lightdm_config2=/etc/lightdm/lightdm-gtk-greeter.conf
    if [[ -x /tmp/"$chroot_path"/usr/bin/lightdm ]] ; then
        echo "[*] Copying $lightdm_config config file..."
        rsync -vaRI "$lightdm_config" /tmp/"$chroot_path"
        rsync -vaRI "$lightdm_config2" /tmp/"$chroot_path"
    fi
